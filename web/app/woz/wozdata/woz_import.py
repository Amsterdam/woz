...
def _get_encoding(filename):
    """
    aangeleverde bestanden blijken niet UTF-8 te zijn, en er is niet aangegeven
    wat wel het import-proces faalde op de aanname dat de bestanden UTF-8 zijn,
    voor 1 van de 4 bestanden. Om het import proces zekerder te maken gebruik ik
    chardet om de encoding van de files te detecteren
    """
...
def _process_csv(csv_file_identification, data_dir, process_row_callback):
    ...
    with open(data_file, "r", encoding=inferred_encoding) as csv_file:
        rows = csv.reader(csv_file, delimiter=';', quotechar=None,
...
def _process_woz_object_row(row):
    ...
    return models.WOZObject(
        ...
        kadastraal_subject_identificatie=row[
            'Kadastraal_subject_identificatie'],
...
def _process_woz_kadastraalobject_row(row):
    ...
    return models.WOZKadastraalObject(
        woz_object=woz_object,
        begindatum_relatie_wozobject=_to_none_or_date(
            row['Begindatum_relatie_wozobject']),
        begindatum_relatie_voorkomen=_to_none_or_date(
            row['Begindatum_relatie_voorkomen']),
        ...
        toegekende_oppervlakte=_to_none_or_integer(
            row['Toegekende_oppervlakte']),
        meegetaxeerde_oppervlakte=_to_none_or_integer(
            row['Meegetaxeerde_oppervlakte'])
    )
...
def _process_woz_waardebeschikking_row(row):
    ...
    return models.WOZWaardeBeschikking(
        woz_object=woz_object,
        begindatum_waarde_object=_to_none_or_date(
            row['Begindatum_waarde_object']),
        einddatum_waarde_object=_to_none_or_date(
            row['Einddatum_waarde_object']),
        begindatum_waarde_voorkomen=_to_none_or_date(
            row['Begindatum_waarde_voorkomen']),
        ...
        begindatum_beschikking_object=_to_none_or_date(
            row['Begindatum_beschikking_object']),
        einddatum_beschikking_object=_to_none_or_date(
            row['Einddatum_beschikking_object']),
        begindatum_beschikking_voorkomen=_to_none_or_date(
            row['Begindatum_beschikking_voorkomen']),
...
def import_woz_files(data_dir):
    ...
    _process_csv('WOZ_kadastraalobject_', data_dir,
                 _process_woz_kadastraalobject_row)
    _process_csv('WOZ_waarde_beschikking_', data_dir,
                 _process_woz_waardebeschikking_row)
